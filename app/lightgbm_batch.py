...
def save_data(df, output_path):
    if output_path.startswith("s3://"):
        local_path = "/tmp/output_lightgbm.csv"
        df.to_csv(local_path, index=False)
        bucket, key = output_path.replace("s3://", "").split("/", 1)
        upload_to_s3(local_path, bucket, key)
    else:
        out_path = output_path.replace("predictions", "predictions_lightgbm")
...
def predict(model, df, predictors):
    proba = model.predict(df[predictors])
...
def main():
    ...
    try:
        ...
        models_to_run = config.get("models_to_run", ["lightgbm"])
        if "lightgbm" not in models_to_run:
            logging.info("LightGBM skipped by config.")
            return
        input_path = config["input_data"]
        output_path = config["yaml_output"]
        model_path = config.get("model_paths", {}).get("lightgbm", config["model_path"])
        ...
        df["prediction_lightgbm"] = predict(model, df, predictors)
        save_data(df, output_path)
        if "Fraud_Flag" in df.columns:
            log_metrics(df["Fraud_Flag"], df["prediction_lightgbm"])
        logging.info("LightGBM batch inference complete.")
    except Exception as e:
        logging.error(f"Error in LightGBM batch: {e}")
