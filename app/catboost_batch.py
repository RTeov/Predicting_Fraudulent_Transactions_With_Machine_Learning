...
def save_data(df, output_path):
    if output_path.startswith("s3://"):
        local_path = "/tmp/output_catboost.csv"
        df.to_csv(local_path, index=False)
        bucket, key = output_path.replace("s3://", "").split("/", 1)
        upload_to_s3(local_path, bucket, key)
    else:
        out_path = output_path.replace("predictions", "predictions_catboost")
...
def load_model(model_path):
    if model_path.startswith("s3://"):
        ...
        download_from_s3(bucket, key, local_path)
        return CatBoostClassifier().load_model(local_path)
    else:
...
def main():
    ...
    try:
        ...
        models_to_run = config.get("models_to_run", ["catboost"])
        if "catboost" not in models_to_run:
            logging.info("CatBoost skipped by config.")
            return
        input_path = config["input_data"]
        output_path = config["yaml_output"]
        model_path = config.get("model_paths", {}).get("catboost", config["model_path"])
        ...
        df["prediction_catboost"] = predict(model, df, predictors)
        save_data(df, output_path)
        if "Fraud_Flag" in df.columns:
            log_metrics(df["Fraud_Flag"], df["prediction_catboost"])
        logging.info("CatBoost batch inference complete.")
    except Exception as e:
        logging.error(f"Error in CatBoost batch: {e}")
