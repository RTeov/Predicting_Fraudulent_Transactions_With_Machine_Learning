...
def save_data(df, output_path):
    if output_path.startswith("s3://"):
        local_path = "/tmp/output_xgboost.csv"
...
def main():
    ...
    try:
        ...
        models_to_run = config.get("models_to_run", ["xgboost"])
        if "xgboost" not in models_to_run:
            logging.info("XGBoost skipped by config.")
            return
        ...
        df["prediction_xgboost"] = predict(model, df, predictors)
        save_data(df, output_path)
        if "Fraud_Flag" in df.columns:
            log_metrics(df["Fraud_Flag"], df["prediction_xgboost"])
        logging.info("XGBoost batch inference complete.")
    except Exception as e:
        logging.error(f"Error in XGBoost batch: {e}")
